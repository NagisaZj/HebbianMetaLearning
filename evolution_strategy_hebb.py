    ...
    def run(self, iterations, print_step=10, path='heb_coeffs'):                                                    
        ...
        for iteration in range(iterations):                                                                         # Algorithm 2. Salimans, 2017: https://arxiv.org/abs/1703.03864
            ...
            if (iteration + 1) % print_step == 0:
                rew_ = rewards.mean()
                print('iter %4i | reward: %.2f |  update_factor: %f  lr: %f | sum_coeffs: %i sum_abs_coeffs: %4i' % (iteration + 1, rew_ , self.update_factor, self.learning_rate, int(np.sum(self.coeffs)), int(np.sum(abs(self.coeffs)))), flush=True)
...
class GradientStrategyHebb(object):
    def __init__(self, hebb_rule, environment, init_weights='uni', population_size=100, sigma=0.1, learning_rate=0.2,
                 decay=0.995, num_threads=1, distribution='normal'):
        ...
        if self.coevolve_init:
            print('\nCo-evolving initial weights of the network')
        network = self.build_network(input_dim,action_dim,self.coevolve_init,self.hebb_rule)
        # Initialize the values of hebbian coefficients and CNN parameters or initial weights of co-evolving initial weights
    ...
    def _get_rewards(self, pool, population):
        ...
        if pool is not None:
            ...
            for p in population:
                ...
                worker_args.append(
                    (self.get_reward, self.hebb_rule, self.environment, self.init_weights, heb_coeffs_try))
        ...
        if pool is not None:
            ...
            for z in range(len(population)):
                ...
                coevolved_parameters_try = np.array(
                    self._get_params_try(self.initial_weights_co, population_coevolved[z]))
                rewards.append(self.get_reward(self.hebb_rule, self.environment, self.init_weights, heb_coeffs_try,
                                               coevolved_parameters_try))
    ...
    def run(self, iterations, print_step=10, path='heb_coeffs'):
        ...
        for iteration in range(iterations):  # Algorithm 2. Salimans, 2017: https://arxiv.org/abs/1703.03864
            ...
            if (iteration + 1) % print_step == 0:
                rew_ = rewards.mean()
                print('iter %4i | reward: %.2f |  update_factor: %f  lr: %f | sum_coeffs: %i sum_abs_coeffs: %4i' % (
